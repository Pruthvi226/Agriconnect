...
public class FarmerWebController {
    ...
    public ModelAndView farmerDashboard(Authentication authentication) {
        ...
        DemandForecastReport forecast = demandForecastService.getLatestForecast();
        mav.addObject("forecast", forecast);
        mav.addObject("forecastTopCrops", forecast.getTopCrops().stream().limit(3).toList());
        Map<String, DemandForecastReport.PriceTrendSnapshot> forecastTrendMap = new LinkedHashMap<>();
        for (DemandForecastReport.PriceTrendSnapshot trend : forecast.getPriceTrends()) {
