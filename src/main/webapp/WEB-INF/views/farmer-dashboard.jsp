...
<div class="container pb-5">
    ...
    <section class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h2 class="h5 fw-800 mb-0"><i class="bi bi-graph-up-arrow text-success me-2"></i> Demand Forecast</h2>
                <span class="badge bg-success-subtle text-success rounded-pill">Next season</span>
            </div>
            <c:choose>
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle mb-0">
                            ...
                            <c:forEach var="crop" items="${forecastTopCrops}">
                                ...
                                <tr>
                                    <td class="fw-bold">${crop.cropName}</td>
                                    <td>${crop.bidCount}</td>
                                    <td>Rs <fmt:formatNumber value="${crop.avgPrice}" minFractionDigits="2" maxFractionDigits="2" />/kg</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${not empty trend and trend.direction == 'UP'}">
                                                <span class="badge bg-success-subtle text-success">Up <fmt:formatNumber value="${trend.changePercent}" maxFractionDigits="2" />%</span>
                                            </c:when>
                                            <c:when test="${not empty trend and trend.direction == 'DOWN'}">
                                                <span class="badge bg-danger-subtle text-danger">Down <fmt:formatNumber value="${0 - trend.changePercent}" maxFractionDigits="2" />%</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="badge bg-secondary-subtle text-secondary">Flat</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                </tr>
                ...
                <c:otherwise>
                    <p class="text-muted mb-0">Forecast data will appear after more bids and accepted prices are available.</p>
                </c:otherwise>
            </c:choose>
        </div>
    </section>
    ...
    <div class="row g-3">
        ...
        <div class="col-6 col-md-3">
            <a href="${pageContext.request.contextPath}/web/farmer/consultations" class="btn btn-white w-100 shadow-sm rounded-4 p-4 border-0 text-center h-100 d-flex flex-column align-items-center justify-content-center">
                <i class="bi bi-camera-video fs-2 text-info mb-2"></i>
                <span class="fw-bold">Expert Help</span>
                <small class="text-muted">Book a consultation</small>
            </a>
        </div>
    </div>
